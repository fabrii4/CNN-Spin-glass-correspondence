...
#output layer
#Magnetization
J1_train=J(f1(Y_test))
J1_test=J(f1(Y_test))


#initialize magnetic field and ground enery
def update_h_E(X, Y, S, S1):
    f1Y=f1(Y)
    Q=len(f1Y)
    i_dim=S.shape[0]
    o_dim=S1.shape[0]
    h_dim=S1.shape[1]
    Z=np.array([[sum(S1[i,l]*f1Y[a,i] for i in range(o_dim)) for l in range(h_dim)] for a in range(Q)])
    T=np.array([[leakyRelu(sum(S[j,l]*X[a,j] for j in range(i_dim))) for l in range(h_dim)] for a in range(Q)])
    f1Z=f1(Z)
    #magnetic fields
    h=np.array([[sum(-2*f1Z[a,l]*X[a,j] for a in range(Q)) for l in range(h_dim)] for j in range(i_dim)])/Q
    h1=np.array([[sum(-2*f1Y[a,i]*T[a,l] for a in range(Q)) for l in range(h_dim)] for i in range(o_dim)])/Q
    #ground energies
    E0=np.array([sum(f1Z[a,l]**2 for a in range(Q)) for l in range(h_dim)])/Q
    E01=np.array([sum(T[a,l]**2 for a in range(Q)) for l in range(h_dim)])/Q
...
S = np.random.rand(input_dim,hidden_dim)*2-1
S1 = np.random.rand(output_dim,hidden_dim)*2-1
...
Nsteps=1000
Nup=200
#beta incremental step
db=(beta_final - beta_in)/Nsteps
#Energy history vectors
E_train=np.zeros(Nsteps)
E_test=np.zeros(Nsteps)

t1_start = time.perf_counter() 

#initialize magnetic fields and ground energies for the 2 layers
h_train, h1_train, E0_train, E01_train = update_h_E(X_train, Y_train, S, S1)
h_test, h1_test, E0_test, E01_test = update_h_E(X_test, Y_test, S, S1)

#Initial energies of the 2 layers
E0 = Energy(J_train, h_train, E0_train, S)
E01 = Energy(J1_train, h1_train, E01_train, S1)


#main loop
for n in range(Nsteps):
    print(f"Step: {n+1}/{Nsteps}\r", end='')
    #reduce temperature
    beta+=db
    ...
    if beta > 1 and E_train[n] > 100:
        beta=beta_in
    #save Energy
    #E_train[n] = E_train[n-1] + dE + dE1 if n > 0 else E0 + E01
    E_train[n] = Energy(J_train, h_train, E0_train, S) + Energy(J1_train, h1_train, E01_train, S1)
    E_test[n] = Energy(J_test, h_test, E0_test, S) + Energy(J1_test, h1_test, E01_test, S1)
    #update spin network
    dE,  dE1 = 0, 0
    for k in range(S.shape[0]*S.shape[1]):
        S, dE_s = Metropolis_step(J_train, h_train, S, beta)
        dE+=dE_s
    for k in range(S1.shape[0]*S1.shape[1]):
        S1, dE1_s = Metropolis_step(J1_train, h1_train, S1, beta)
        dE1+=dE1_s
    #update magnetic fields and ground energies for the 2 layers
    if n > 0 and n % Nup == 0:
        h_train, h1_train, E0_train, E01_train = update_h_E(X_train, Y_train, S, S1)
        h_test, h1_test, E0_test, E01_test = update_h_E(X_test, Y_test, S, S1)
...
hid=np.zeros((len(X_test),hidden_dim))
for a in range(len(X_test)):
    #first layer
    for l in range(hidden_dim):
        hid[a,l]=leakyRelu(sum(w[j,l]*X_test[a,j] for j in range(input_dim)))
    for i in range(output_dim):
        out[a,i]=leakyRelu(sum(w1[l,i]*hid[a,l] for l in range(hidden_dim)))
for i in range(10):
